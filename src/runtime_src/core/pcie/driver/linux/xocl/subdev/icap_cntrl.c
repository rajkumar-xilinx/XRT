...
{
	struct icap_cntrl *ic = platform_get_drvdata(to_platform_device(dev));
	u32 val = READ_REG32(ic, ICAP_PROGRAMMING_REG);
...
//TODO: Read version control register 0x330000 and check if MSB bit is set.
//Enable icap controller programming only when this MSB bit is set.
