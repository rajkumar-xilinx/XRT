...
#include "../xocl_drv.h"
#include "../xocl_drm.h"
#include "../lib/libqdmacpm4/libqdma_export.h"
#include "../lib/libqdmacpm4/qdma_ul_ext.h"
...
/* Module Parameters */
unsigned int qdma_cpm4_max_channel = 16;
module_param(qdma_cpm4_max_channel, uint, 0644);
MODULE_PARM_DESC(qdma_cpm4_max_channel, "Set number of channels for qdma, default is 16");

static unsigned int qdma_cpm4_interrupt_mode = DIRECT_INTR_MODE;
module_param(qdma_cpm4_interrupt_mode, uint, 0644);
MODULE_PARM_DESC(interrupt_mode, "0:auto, 1:poll, 2:direct, 3:intr_ring, default is 2");

struct dentry *qdma_cpm4_debugfs_root;
...
static long qdma_cpm4_stream_ioctl_create_queue(struct xocl_qdma *qdma,
			void __user *arg)
{
...
static long qdma_cpm4_stream_ioctl_alloc_buffer(struct xocl_qdma *qdma,
			void __user *arg)
{
...
static long qdma_cpm4_stream_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct xocl_qdma *qdma;
	long result = 0;
	...
	case XOCL_QDMA_IOC_CREATE_QUEUE:
		result = qdma_cpm4_stream_ioctl_create_queue(qdma, (void __user *)arg);
		break;
	case XOCL_QDMA_IOC_ALLOC_BUFFER:
		result = qdma_cpm4_stream_ioctl_alloc_buffer(qdma, (void __user *)arg);
		break;
	default:
		xocl_err(&qdma->pdev->dev, "Invalid request %u", cmd & 0xff);
		result = -EINVAL;
...
static const struct file_operations qdma_stream_fops = {
	...
	.unlocked_ioctl = qdma_cpm4_stream_ioctl,
};

static int qdma_cpm4_csr_prog_ta(struct pci_dev *pdev, int bar,
...
static int qdma_cpm4_probe(struct platform_device *pdev)
{
	...
	conf->qdma_drv_mode = qdma_cpm4_interrupt_mode;
	...
	if (csr_bar >= 0) {
		...
		ret = qdma_cpm4_csr_prog_ta(XDEV(xdev)->pdev, csr_bar, csr_base);
		if (ret < 0)
			xocl_err(&pdev->dev,
	...
	if (!XOCL_DSA_IS_SMARTN(xdev)) {
		ret = set_max_chan(qdma, qdma_cpm4_max_channel);
		if (ret) {
...
static int qdma_cpm4_remove(struct platform_device *pdev)
{
...
struct xocl_drv_private qdma_cpm4_priv = {
	.ops = &qdma_ops,
...
static struct platform_driver	qdma_cpm4_driver = {
...
int __init xocl_init_qdma_cpm4(void)
{
	...
	qdma_cpm4_debugfs_root = debugfs_create_dir("qdma_cpm4_dev", NULL);
	if (!qdma_cpm4_debugfs_root) {
		pr_err("%s: Failed to create debugfs\n", __func__);
		return -ENODEV;
	}

	err = libqdma_init(0, qdma_cpm4_debugfs_root);
	if (err)
		return err;
	...
	err = platform_driver_register(&qdma_cpm4_driver);
	if (err)
		goto err_drv_reg;
...
err_reg_chrdev:
	...
	if (qdma_cpm4_debugfs_root) {
		debugfs_remove_recursive(qdma_cpm4_debugfs_root);
		qdma_cpm4_debugfs_root = NULL;
	}
...
{
	unregister_chrdev_region(str_dev, XOCL_CHARDEV_REG_COUNT);
	platform_driver_unregister(&qdma_cpm4_driver);
	libqdma_exit();
	if (qdma_cpm4_debugfs_root) {
		debugfs_remove_recursive(qdma_cpm4_debugfs_root);
		qdma_cpm4_debugfs_root = NULL;
